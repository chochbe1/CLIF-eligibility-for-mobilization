...
paths_weekday <- list(
  ...
  Green  = file.path(root, "..", "output", "intermediate",
                     "competing_risk_green_final_weekday.parquet")
)
...
cat("\nKey outputs for weekday sensitivity:\n")
cat("- weekday_sensitivity_comparison_72hrs.csv\n") 
cat("- median_times_72hrs_weekday.csv\n")
cat("- cif_comparison_allday_vs_weekday_72hrs.png\n")

# Print final comparison summary
cat("\n=== MEDIAN TIME DIFFERENCES (WEEKDAY vs ALL-DAY) ===\n")
if (exists("comparison_72h")) {
  for (i in 1:nrow(comparison_72h)) {
    cat(sprintf("%s: %.1f hrs (all-day) → %.1f hrs (weekday) | Diff: %+.1f hrs (%.1f%%)\n",
                comparison_72h$Criterion[i],
                comparison_72h$Median_AllDays[i], 
                comparison_72h$Median_Weekdays[i],
                comparison_72h$Difference_Hours[i],
                comparison_72h$Percent_Change[i]))
  }
}
